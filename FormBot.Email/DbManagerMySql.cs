...
namespace FormBot.Email
{
	...
	{
		public MySqlDbManager() : this((Account) null) {}
		...
		public MySqlDbManager(string dataFolder)
		{
			_dataFolder = dataFolder;
			_connection = new OdbcConnection();
			_commandCreator = new MySqlCommandCreator(_connection as OdbcConnection, new OdbcCommand(), dataFolder);
		}
		...
		public override void Connect()
		{
            ...
            try
            {
                ExecuteScalarCommand(_commandCreator.SetNames());
            }
            catch
            {
            }
			...
			foreach (DataRow row in schemaTable.Rows)
			{
				...
				switch (fieldName.ToLower(CultureInfo.InvariantCulture))
				{
                    ...
                    case "imap_quota":
                        ...
                        break;
                    case "namespace":
                        acct.Namespace = Convert.ToString(reader.GetValue(index));
                        break;
                }
			}
			return acct;
		...
		protected override UserSettings ReadUserSettings(IDataReader reader)
		{
			...
			if (_dataFolder != string.Empty)
			{
				settings = new UserSettings(_dataFolder);
			}
			else
			{
				...
				if (fieldName != null)
				{
					switch (fieldName.ToLower(CultureInfo.InvariantCulture))
					{
                        ...
                        case "auto_checkmail_interval":
                            settings.AutoCheckmailInterval = Convert.ToInt32(reader.GetValue(index));;
                            break;
                    }
				}
			}
				...
				if (fieldName != null)
				{
					switch (fieldName.ToLower(CultureInfo.InvariantCulture))
					{
						...
						case "id_folder":
							...
							break;
						case "applied":
							result.Applied = Convert.ToBoolean(reader.GetValue(index));
							break;
					}
				}
			}
				...
				if (fieldName != null)
				{
					switch (fieldName.ToLower(CultureInfo.InvariantCulture))
					{
						...
						case "forwarded":
							...
							break;
						case "flags":
                            webMsg.Flags = (SystemMessageFlags)Convert.ToInt32(reader.GetValue(index));
							break;
						case "body_text":
						...
						case "charset":
							...
							break;
                        case "sensitivity":
                            webMsg.Sensitivity = (WebMailSensitivity)Convert.ToInt16(reader.GetValue(index));
                            break;
                    }
				}
			}
                ...
                if (fieldName != null)
                {
                    switch (fieldName.ToLower(CultureInfo.InvariantCulture))
                    {
                        ...
                        case "allow_users_change_timezone":
                            ...
                            break;
                        case "msgs_per_page":
                            dom.Msgs_per_page = Convert.ToInt16(reader.GetValue(index));
                            break;
                        case "skin":
                        ...
                        case "global_addr_book":
							dom.Global_addr_book = Convert.ToBoolean(reader.GetValue(index));
							break;
                        case "view_mode":
                            dom.ViewMode = (ViewMode)Convert.ToByte(reader.GetValue(index));
                            break;
                        case "save_mail":
                            dom.SaveMail = (SaveMail)Convert.ToByte(reader.GetValue(index));
                            break;
                    }
                }
            }
