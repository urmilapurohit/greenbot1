	...
	public class Utils
	{
        ...
        public static int GetTrialDaysLeft(string licenseKey)
        {
            ...
            try
            {
                ...
                Pop3.LicenseKey = FormBot.Email.Constants.LiteLicenseKey;
                Pop3 pop = new Pop3();
		...
		public static string GetTempFolderName()
		{
            ...
            if (HttpContext.Current.Session != null)
			{
				...
				if (acct != null)
				{
					tempFolder = GetMD5DigestHexString(HttpContext.Current.Session.SessionID);
                ...
                if (HttpContext.Current.Session[Constants.sessionTempFolder] != null)
				{
                    if (string.Compare(HttpContext.Current.Session[Constants.sessionTempFolder].ToString(), tempFolder, true, CultureInfo.InvariantCulture) != 0)
					{
                        if (Directory.Exists(HttpContext.Current.Session[Constants.sessionTempFolder].ToString()))
						{
                            Directory.Delete(HttpContext.Current.Session[Constants.sessionTempFolder].ToString(), true);
						}
					}
                    HttpContext.Current.Session[Constants.sessionTempFolder] = tempFolder;
				}
				else
				{
                    HttpContext.Current.Session.Add(Constants.sessionTempFolder, tempFolder);
				}
			}
			else
			{
        ...
        public static string GetTempFolderNameForEmailThread(string email, int acct_id)
        {
            string tempFolder = string.Empty;
            tempFolder = GetMD5DigestHexString(HttpContext.Current.Session.SessionID);
            FileSystem fs = new FileSystem(email, acct_id, false);
            tempFolder = fs.CreateFolder(tempFolder);
            return tempFolder;
        }
		...
		public static string GetMessageHtmlReplyToBody(Account acct, MailMessage mailMessage)
		{
            ...
            //string mess = @"<br/><br/>" + signature + @"</br>";
        ...
        public static string GetAttachmentDownloadLink(MailMessage msg, Attachment attach, string full_name_folder, bool isViewLink)
        {
            string attachmentName = (attach.Filename.Length > 0) ? attach.Filename : attach.Name;

            string partID = attach.Headers["X-PartID"];

            string tmp_filename = Utils.CreateTempFilePath(Utils.GetTempFolderName(), attachmentName, true);
        ...
        public static string GetDataFolderPath()
        {
            ...
            strDataFolderPath += "/EmailManagement";
