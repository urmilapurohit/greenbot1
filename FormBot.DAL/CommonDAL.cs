...
namespace FormBot.DAL
{
    public static class CommonDAL
    {
        private static string DefaultConnectionString = Convert.ToString(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
        ...
        public static List<T> ExecuteProcedureDapper<T>(string procedureName, params DynamicParameters[] commandParameters)
        {
            var data = new List<T>();
            using (IDbConnection con = new SqlConnection(DefaultConnectionString))
            {
                var task = con.QueryMultiple(procedureName, commandParameters[0], commandType: CommandType.StoredProcedure, commandTimeout: 3000);
                data = task.Read<T>().ToList();
            }
            return data;
        }
        public static JobViewLists ExecuteProcedureJobViewDapper(string procedureName, params DynamicParameters[] commandParameters)
        {
            var data = new JobViewLists();
            using (var con = new SqlConnection(DefaultConnectionString))
            {
                var task = con.QueryMultiple(procedureName, commandParameters[0], commandType: CommandType.StoredProcedure, commandTimeout: 3000);
                data.jobViews = task.Read<JobView>().ToList();
                data.userWiseColumns = task.Read<UserWiseColumns>().ToList();
            }
            return data;
        }
    }
