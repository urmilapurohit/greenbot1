        ...
        public static StringBuilder DynamicMenuHtml(HtmlHelper htmlHelper, List<MenuView> menus, List<MenuView> menuDetails, List<MenuView> menuDeailsWithoutSubMenu)
        {
            ...
            foreach (var menu in menus)
            {
                ...
                if (menu.WithoutSubMenu)
                {
                    ...
                    {
<<<<<<< HEAD
                        string image = "<img src=\"" + urlHelper.Content("~/Images/") + menu.Image + "\" alt='Create New' />";
                        url = urlHelper.Action(menuDetail.Action, menuDetail.Controller);
                        output.Append(@"<li> <a class='navlink' href='" + url + "' title='Create New'> <i class='icon'> " + image + " </i> " + menu.DisplayName + " </a></li>");
=======
...
<<<<<<< HEAD
                    string image = "<img src=\"" + urlHelper.Content("~/Images/") + menu.Image + "\" alt='Create New' />";
                    //string image =  "<img src='"+HttpContext.Current.Request.UrlReferrer + "images/create-icon.png' />";
                    if (menu.DisplayName == "Users")
                        output.Append(@"<li class='active'> <a class='navlink' href='javascript:void(0)' title='Create New'> <i class='icon'> " + image + " </i> " + menu.DisplayName + " </a>");
                    else
                        output.Append(@"<li> <a class='navlink' href='#' title='Create New'> <i class='icon'> " + image + " </i> " + menu.DisplayName + " </a>");
=======
                    var image = ProjectSession.SiteUrlBase + menu.Image;
                    if (menu.DisplayName == "Users")
