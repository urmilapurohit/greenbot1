...
var FilterMenu = function () {
    ...
    FilterMenu.prototype.initialize = function () {
        ...
        //this.th.prepend(this.menu);
        this.th.insertBefore(this.menu, this.th.childNodes[0]);
        ...
        $trigger.click(function (event) {
            ...
            //$('#datatable thead tr th:nth-child(2) .dropdown-filter-content').addClass('column-2-popup');
            //return $content.toggle();

            //var len = $(".dropdown-filter-content:visible").length;
            //debugger;
            //if (len > 0) {
            //    debugger;
            //    $(".dropdown-filter-content").removeAttr("style");
            //    return $content.toggle();
            //}
            //else
            //    return $content.toggle();
            ...
            $('#datatable thead tr th:nth-child(2) .dropdown-filter-content').addClass('column-2-popup');            
            var len = $(".dropdown-filter-content:visible").length;
            //var len1 = $(".dropdown-filter-content activee").length;
            //var len2 = $content.length;
            if (len > 0) {
                var observer = new MutationObserver(function (mutations) {
                    mutations.forEach(function (mutationRecord) {
                        if ($(".activee").css('display') == 'none') {
                            $(this).removeClass("activee");
                        }
                    });
                });
                var menu = $(this).parent();
                var self = menu.find("div");
                if (self.hasClass("activee")) {
                    self.removeClass("activee")
                    return $content.toggle();
                }
                else {                    
                    $(".dropdown-filter-content").removeAttr("style");
                    $(".dropdown-filter-content").removeClass("activee");
                    $content.addClass("activee");
                    var target = document.getElementsByClassName("activee")[0];
                    var a = target.length;
                    if (a == 0) {
                        //alert(0);
                    }
                ...
                var observer = new MutationObserver(function (mutations) {
                    mutations.forEach(function (mutationRecord) {
                        if ($(".activee").css('display') == 'none') {
                            $(".dropdown-filter-content:hidden").removeClass("activee");
                        }
                    });
                });
                $content.addClass("activee");
    ...
    FilterMenu.prototype.dropdownFilterSearch = function () {        
        ...
        input.setAttribute('data-column', this.column.toString());
        input.setAttribute('data-index', this.index.toString());
        input.setAttribute('placeholder', "Search" + " " + this.th.innerText);

        var clear = document.createElement('button');
        clear.className = 'glyphicon glyphicon-remove-circle searchclear';
        clear.type = 'reset'

        //var hr = document.createElement('hr');
        //hr.className='hrsearch'
        //dropdownFilterItem.appendChild(hr);

        dropdownFilterItem.appendChild(input);
        dropdownFilterItem.appendChild(clear);
    ...
    //   // if (span.className == 'a-to-z') {
    //   //     img.setAttribute('src', '../Images/AtoZ.png');
    //   // }
    //   // else if (span.className == 'z-to-a') {
    //   //     img.setAttribute('src', '../Images/ZtoA.png');
    //   // }
    //   // else{
    //   // }
    //   // dropdownFilterItem.appendChild(img);
    ...
    FilterMenu.prototype.dropdownFilterDropdown = function () {        
       ...
       // arrow.className = 'pointer';
        ...
        icon.className = 'glyphicon glyphicon-filter pull-right';
...
var FilterCollection = function () {
    ...
    FilterCollection.prototype.bindSearch = function () {
        ...
        //    }
        //    var that = this;
        //    $('.dropdown-filter-search input').keyup(function () {
        //        jobIndexTable.column($(this).parent().parent().parent().parent().index() + ':visible').search(this.value).draw();
        //    });
        //});
        ...
        $(".searchclear").click(function () {;
            if ($('.dropdown-filter-search').is(':visible') == true) {
                $('input:text').val('');
                jobIndexTable.column($(this).parent().parent().parent().parent().index() + ':visible').search('').draw();
            }
        });
    };
