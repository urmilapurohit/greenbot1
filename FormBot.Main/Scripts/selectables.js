...
function Selectables(opts) {
    ...
    this.rectOpen = function (e) {
        if (oldTimeStamp != e.timeStamp) {
            call = 0;
            oldTimeStamp = e.timeStamp
        }
        if (!($(e.target).closest('div').hasClass('added')) && !($(e.target).closest('div').hasClass('document_imageUpload')) && !($(e.target).closest('div').is('[class*="groupingDiv"]')) && !($(e.target).closest('div').is('[class*="image-ar"]')) && !($(e.target).closest('div').is('[class*="ui-resizable"]')) && !($(e.target).closest('div').is('[class*="nicescroll"]'))) {
            ...
            if (call == 0 && (e.timeStamp == oldTimeStamp)) {
                InputControlClick($(e.target).closest('div'),e);
                call = 1;
            }
        }
    };
    ...
    var cross = function (a, b) {
        ...
        return !(((aTop + a.offsetHeight) < (bTop)) || (aTop > (bTop + (b.offsetHeight * ($("#zoom-dropdown").val() / 100)))) || ((aLeft + a.offsetWidth) < bLeft) || (aLeft > (bLeft + (b.offsetWidth * ($("#zoom-dropdown").val() / 100)))));
    };
