...
function FillDropDownKendo(id, url, value, hasSelect, callback, defaultText = "", onChange = null) {
    $.ajax(
        {
            url: url,
            contentType: 'application/json',
            ...
            success: function (success) {
                ...
                $.each(success, function (i, val) {
                    //options += '<option value = "' + val.Value + '" >' + val.Text + '</option>'
                    options.push({ "value": val.Value, "text": val.Text });
                });
...
$(function () {
    ...
    //SE signature upload
    $('#uploadBtnSignatureSE').fileupload({
        url: url,
        dataType: 'json',
        done: function (e, data) {
            var UploadFailedFiles = [];
        ...
        dataType: 'json',
            ...
            for (var i = 0; i < data.result.length; i++) {
                ...
                if (data.result[i].Status == true) {
                    ...
                    if (mimeType == "image") {
                        content += '<td  class="col-sm-2" style="color:blue"><img src=' + ProjectImagePath + 'images/view-icon.png style="cursor: pointer" id="' + data.result[i].FileName.replace("%", "$") + '"  class="' + data.result[i].FileName.replace("%", "$") + '" title="Preview" onclick="OpenDocument(this)" proofDocumentType="' + data.result[i].proofDocumentType + '"></td>';
                    }
                    else {
                        content += '<td  class="col-sm-2" style="color:blue"><img src=' + ProjectImagePath + 'images/view-icon.png style="cursor: pointer" id="' + data.result[i].FileName.replace("%", "$") + '" class="' + data.result[i].FileName.replace("%", "$") + '" title="Download" onclick="DownloadDocument(this)" proofDocumentType="' + data.result[i].proofDocumentType + '"></td>';
                    }
        ...
        dataType: 'json',
            ...
            for (var i = 0; i < data.result.length; i++) {
                ...
                if (data.result[i].Status == true) {
                    ...
                    //$('#imgSign').attr('src',$('#imgSign').attr('src', SRCSign).load(function () { logoWidth = this.width; logoHeight = this.height });
...
function OpenDocument(e) {
    ...
    if (typeof (e.attributes.proofdocumenttype) != 'undefined' && (typeof (e.attributes.userdocumentid) == 'undefined' || parseInt(e.attributes.userdocumentid.value) > 6124)) {
        ...
        else if (e.attributes.proofdocumenttype.value == 5) {
            debugger;
            proofDocumentType = "/" + "InstallersDriversLicense";
        }
    }
...
function DownloadDocument(e) {
    ...
    //$("#loading-image").show();
    var foldername = USERID;
    if (typeof (e.attributes.proofdocumenttype) != 'undefined' && (typeof (e.attributes.userdocumentid) == 'undefined' || parseInt(e.attributes.userdocumentid.value) > 505)) {
        if (e.attributes.proofdocumenttype.value == 1) {
            ...
            foldername = foldername + "\\" + "InstallersDriversLicense";
        }
    }
    var FileName = e.id;
    window.location.href = BaseURL + 'ViewDownloadFile/User?FileName=' + e.className + '&FolderName=' + foldername;
...
function DeleteContractFileFromDatabase(InvoicerID, DocumentPath) {
    if (confirm('Are you sure you want to delete this file?')) {
        var documentpath = DocumentPath;
        $.ajax(
            {
                url: BaseURL + 'DeleteContractFileByID/User',
                data: { InvoicerID: InvoicerID, Documentpath: documentpath },
                contentType: 'application/json',
                method: 'get',
                success: function () {
...
function DeleteFileFromDatabase(UserDocumentID, userId, documentpath, documentType) {
    ...
    //5360 = last UserDocumentId before sca mulitple proof category (staging)
    //6124 = last UserDocumentId before sca mulitple proof category (live)
    if (typeof (documentType) != 'undefined' && (UserDocumentID > 505 || documentType == 6)) {
        if (documentType == 1) {
            ...
            documentpath = "InstallersDriversLicense" + "\\" + documentpath;
            tableId = 'tblInstallerDriversLicense';
        }
        else if (documentType == 6) {
            documentpath = documentpath;
            tableId = "tblContractPath";
        }
    }
...
function DeleteFileFromFolder(fileDelete, documentType) {
    ...
    if (typeof (documentType) != 'undefined') {
        if (documentType == 1) {
            ...
            fileDelete = "InstallersDriversLicense" + "\\" + fileDelete;
            tableId = 'tblInstallerDriversLicense';
        }
        else if (documentType == 6) {
            tableId = "tblContractPath";
...
function deleteImage(imageId) {
    ...
    var fileDelete = $('#' + imageId).attr('class');
    if (confirm('Are you sure you want to delete this file ?')) {
        $.ajax(
            {
                url: BaseURL + 'DeleteSignatureFileFromFolder/User',
                ...
                method: 'get',
                success: function () {
                    var sign = $('#' + imageId).attr('class');
                    $("[name='Signature']").each(function () {
                        $(this).remove();
                    });
                    $('#' + imageId).removeAttr('src');
                    $('#' + imageId).removeAttr('class');
                    if (imageId.toLowerCase() == "imgsign") {
                        $('#popupbox').modal('hide');
                        $("#imgSignature").hide();
                    }
                    else {
                        $('#popupboxSE').modal('hide');
                        $("#imgSignatureSE").hide();
                    }
...
function deleteSelfieImage(imageId, UserId) {
    var FolderName = USERID;
    var OldFileName = oldFileNameSelfie;
    var fileDelete = $('#imgSelfieImage').attr('class');
    if (confirm('Are you sure you want to delete this file ?')) {
        $.ajax(
            {
                url: BaseURL + 'DeleteSelfieFileFromFolder/User',
                data: { fileName: fileDelete, FolderName: FolderName, OldFileName: oldFileNameSelfie, UserId: UserId },
                contentType: 'application/json',
                method: 'get',
                success: function () {
                    var sign = $('#imgSelfieImage').attr('class');
                    $("[name='Selfie']").each(function () {
                        $(this).remove();
                    });
                    $('#imgSelfieImage').removeAttr('src');
                    $('#imgSelfieImage').removeAttr('class');
                    $('#popupboxSelfie').modal('hide');
                    $("#imgSelfie").hide();
                    $(".alert").hide();
                    $("#successMsgRegion").html(closeButton + "File has been Deleted successfully.");
                    $("#successMsgRegion").show();
...
function DeleteFileFromSelfieOnUpload(fileNames, guid, oldFileName) {
    $.ajax(
        {
            url: BaseURL + 'DeleteSelfieFileFromFolder/User',
            data: { fileName: fileNames, FolderName: guid, OldFileName: oldFileNameSelfie, UserId: 0 },
            contentType: 'application/json',
            method: 'get',
            success: function (data) {
