$(document).ready(function () {  
    window.mapsCallback = function () {
        getLatitudeLongitude(DisplayLatLonOfInstallationAdd, $("#dMap").html());
    };
...
$('#uploadBtnPhoto').fileupload({
    ...
    dataType: 'json',
        ...
        for (var i = 0; i < data.result.uploadStatus.length; i++) {
            ...
            //$("#ddlviewphotosName").append($("<option></option>").val(path).attr('data-vclid', data.result.uploadStatus[i].AttachmentID).html(data.result.uploadStatus[i].FileName));
            var Newpath = path;//.replaceAll("\\", "\\\\");
            var completePath = uploadPath + Newpath;
            var onclickstr = "viewimgonPage('" + completePath + "','" + data.result.uploadStatus[i].AttachmentID +"')";
            $("#ddlviewphotosName").append($("<option></option>").val(completePath).attr('data-vclid', data.result.uploadStatus[i].AttachmentID).html(data.result.uploadStatus[i].FileName));
...
function downloadAllAsZipForTabular(isdwnlAll) {
    ...
    if (allData.vp.length > 0 || allData.rp.length > 0) {
        showStayMsg = false;
        window.location.href = urlDownloadJobPhotosAllTabular + "?jobid=" + jobid + "&photos=" + encodeURIComponent(JSON.stringify(allData)) ;
        SearchHistory();
    }
    else {
        showStayMsg = true;
...
function downloadAllAsZip(isdwnlAll) {
    ...
    $.each(mainPnl, function (i, e) {
        var obj = {}, checkListItems = [];
        obj.pl = [];
        var $pnl = $(this).find('[id*=ChecklistItmes]').children();
        obj.id = $(this).find('.panel-heading').find('a').data('name');
        $.each($pnl, function (i, e) {
            var pnlCheck = {};
            pnlCheck.p = [];
...
function lnkDwnldAll_click() {    
    downloadAllAsZipForTabular(1);
}
function uploadBtnPhoto_click() {
    
    var $checkedInput = $('#divVisitList').find('[data-IsDeleted="False"]').find('.parentInput:checked').add($('#headingReference').find('input:checked')).add($('.defaultPanel:checked')).add($('.headingReference:checked'));
    var isRef = false;
    isRef = ($('.headingReference:checked').length > 0) ? true : false;
    isDefault = ($('.defaultPanel:checked').length > 0) ? true : false;
                    ...
                    if (isFromPhotoView != 1) {
                        var a = setTimeout(function () {
                            if (ProjectSession_UserTypeId == 1) {
                                geocodeAddress($('#txtDestinationAddForMap').attr("data-id"));
                            }
                        }, 1000);
                    }
...
function showMap(lat, lon) {
    ...
    $('#txtDestination').val($('#txtDestinationAddForMap').attr("data-id"));
    ...
    function createLine() {
        ...
        var temp, temp2;
                ...
                if (address2) {
                    ...
                    location1 = convertLocationToLatLong($("#txtSource").val());
                    plotMap(location1);
                }
...
function ReloadJobPhotoSection1(jobId) {
    ...
    setTimeout(function () {
        $.get(urlreloadphoto + jobId, function (data) {
            $('#loadallPhotos').empty();
            $('#loadallPhotos').append(data.photoView);
            showMessageForJobPhoto("File has been restored successfully.", false);
        });
    }, 500);
}
...
function fnLoadReferencePhotos() {
    
    $('.refPhotos').empty();
    $.ajax({
        type: 'GET',
        url: '/Job/LoadReferencePhotos',
        data: { JobID: JobId },
        dataType: "json",
        contenttype: 'application/json; charset=utf-8',
        async: true,
        success: function (data) {
            ...
            $('.refPhotos').append(data.referencephotoView);
        },
        error: function (xhr) {
            alert('error');
        }
...
function fnLoadDefaultPhotos() {
    
    $('.defPhotos').empty();
    $.ajax({
        type: 'GET',
        url: '/Job/LoadDefaultPhotos',
        data: { JobID: JobId },
        dataType: "json",
        contenttype: 'application/json; charset=utf-8',
        async: true,
        success: function (data) {
            ...
            $('.defPhotos').append(data.defaultphotoView);
        },
        error: function (xhr) {
            alert('error');
        }
...
function showLocationofPhoto () {
    debugger
    var lat = $('#showlocationofPhoto').attr('data-lat');
    var lon = $('#showlocationofPhoto').attr('data-lon');
    showMap(lat, lon);
}
$('#showlocationofPhoto').click(function () {
    debugger
