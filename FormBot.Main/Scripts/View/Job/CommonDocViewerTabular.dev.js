...
function renderPage(pageNum, xOffset, yOffset, renderAcroFieldSelect) {
    ...
    pdfDoc.getPage(pageNum).then(function (page) {
        ...
        var viewport = page.getViewport(viewportWidth/1350);
...
function setupForm(div, content, viewport) {
    ...
    content.getAnnotations().then(function (items) {
        for (var i = 0; i < items.length; i++) {
            ...
            //if (items[i].fullName[0] == 'Installer_Signature_1' || items[i].fullName[0] == 'undefined_258') {
            //}
    ...
    $('#btnSave').click(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                showMessageViewer("Document has been saved successfully.", false);
                signDetail.map(signature => signature.IsSave = true)
            },
            error: function (result) {
                showMessageViewer("Document has not been saved.", true);
            }
                    ...
                    if (data.Type.toString() == texthashCode || data.Type.toString() == buttonhashCode) {
                        ...
                        if (data.FieldName.includes("IsSameAsOwnerAddress")) {
                            ...
                            if ($('input[name="' + data.FieldName + '"]').length == 1) {
                                $.each($('input[name="' + data.FieldName + '"]'), function (i) {
                                    //var len = $('[id^=' + data.FieldName + ']').length;
...
function showMessageViewer(msg, isError) {
    $(".alert").hide();
    var visible = isError ? "errorMsgRegionViewer" : "successMsgRegionViewer";
    var inVisible = isError ? "successMsgRegionViewer" : "errorMsgRegionViewer";
    $("#" + inVisible).hide();
